...
The ASME VIII-2 and IOGP dispatcher endpoints resolve helper inputs from `FlangedJoint` where the project has selected gasket `m/y` data. They also trace selected material allowable stress at the load-case temperature when the `MaterialSnapshot` already contains that resolved property. Dispatcher trace quantities intended for users are reported in MPa, mm, mm2, and DegC notes. These endpoints return numeric trace quantities but remain `PartiallyImplemented`; they do not yet represent complete validated or qualified code procedures.
